...
export function PlaceholdersAndVanishInput({
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        if (e.key === "Enter" && !e.shiftKey && !animating) {
            ...
            if (inputRef.current) {
                ...
                inputRef.current.form?.dispatchEvent(formEvent);
            }
        }
    }
    ...
    const adjustTextareaHeight = (element: HTMLTextAreaElement) => {
        ...
        element.style.height = element.scrollHeight > element.clientHeight
            ? `${Math.min(element.scrollHeight, 300)}px`
            : "50px";
    }
    ...
    }, [inputRef, adjustTextareaHeight])

    return (
        <form
            className={cn(
                "w-full relative max-w-3xl mx-auto bg-white dark:bg-zinc-800 rounded overflow-hidden shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),_0px_1px_0px_0px_rgba(25,28,33,0.02),_0px_0px_0px_1px_rgba(25,28,33,0.08)] transition duration-200",
                value && "bg-gray-50",
            )}
            style={{
                minHeight: "2rem",
